...
void main() {
  group('native integration', () {
    ...
    test(
      'converts to and recompresses every supported output format',
      () async {
    ...
    test('supports target-size compression', () async {
      final CaesiumMemoryResult result = await FlutterCaesiumFfi.compressToSize(
        png,
        maxOutputBytes: 128,
        returnSmallest: true,
      );
    ...
    test('supports resize and lossless WebP options', () async {
      ...
      final CaesiumMemoryResult lossless = await FlutterCaesiumFfi.convert(
        png,
        format: CaesiumFormat.webp,
        options: const CaesiumOptions(webp: WebpOptions(lossless: true)),
      );
    ...
    test('removes or preserves EXIF metadata as configured', () async {
      ...
      final CaesiumMemoryResult removed = await FlutterCaesiumFfi.compress(
        jpegWithExif,
      );
      ...
      await expectLater(
        ...
        throwsA(
          isA<CaesiumException>()
              .having((CaesiumException error) => error.code, 'code', isNot(0))
              .having(
    ...
    test('writes files and protects existing output', () async {
      ...
      final CaesiumFileResult compressed = await FlutterCaesiumFfi.compressFile(
        input.path,
        compressedPath,
      );
